...
if [ $# -ne 3 ]; then
    echo "Usage: recipes/05_create_final_kaldi_files.sh <input_dir> <output_dir> <corpus_dir>"
    echo "e.g.:  $0 generated/autumn2020 final_outputs/fi-parl-autumn2020 path/to/corpus"
    echo
...
for f in $(find "${INPUT}" -type f -name "*segments.new" | sort); do
    sort -uo $f{,}
done
...
for f in $(find "${INPUT}" -type f -name "*text.new" | sort); do
    sort -uo $f{,}
done
